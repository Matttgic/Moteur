...
export type Opportunity={id:string;eventId:string;sport:string;event:string;homeTeam:string;awayTeam:string;commenceTime:string;market:string;selection:string;point:number|null;bookmaker:string;bookmakerTitle:string;offeredOdds:number;referenceOdds:number;fairOdds:number;fairProbability:number;edge:number;confidence:number;reference:string;referenceUpdatedAt:string|null;bookmakerUpdatedAt:string|null};
